...
def convert_into_bag(data, bag_indexs,requires_grad=True, pad_val = 0.0, counts = None):
    """
    Convert instances into bag

    Args:
        data: individual tensors
        bag_indexs: bag_indices
        bag_type: 1 - max type, 0 - mean
        confidence: None
    Returns:
        bag: tensor
    """
    # # bag = []
    # bag_size = max(bag_indexs).int().item() + 1
    # bag = torch.zeros(bag_size,data.shape[-1],requires_grad=requires_grad).type_as(data)
    # if confidence is not None:
    #     bag_confidence = torch.zeros(bag_size,data.shape[-1],requires_grad=requires_grad).type_as(confidence)
    # for local_index in range(bag_size):
    #     sub_data = data[bag_indexs == local_index, :]
    #     if confidence is not None:
    #         sub_confidence = confidence[bag_indexs == local_index, :]
    #     if sub_data.shape[0] > 0:
    #         bag[local_index,:] = sub_data.max(0)[0] if bag_type else sub_data.mean(0)
    #         if confidence is not None:
    #             bag_confidence[local_index,:] = torch.gather(sub_confidence, 0, sub_data.argmax(dim=0,keepdim=True))
    #     # bag.append(data[bag_indexs == local_index, :].max(0)[0])
    # # bag = torch.stack(bag,dim=0)
    # if confidence is not None:
    #     return bag, bag_confidence
    # return bag
    counts = torch.bincount(bag_indexs.type(torch.int)) if counts is None else counts
    bag_data = nn.utils.rnn.pad_sequence(data.split(tuple(counts.tolist()), dim=0), batch_first=True,padding_value=pad_val)
    bag_data = bag_data if requires_grad else bag_data.requires_grad_(False)
    return bag_data


def get_orig_pos_neg(data,bag_indxs,valid_indexs,cfg=None):
    orig_data = []
    neg_data = []
    ninst_count = []
    bag_size = max(bag_indxs).int().item() + 1
    for local_index in range(bag_size):
        ninst = (valid_indexs == local_index).sum()
        sub_data = data[bag_indxs == local_index, :]
        orig_data.append(sub_data[:ninst, :])
        ninst_count = ninst_count + [ninst] * ninst
        if cfg is not None and cfg.AVA.PERTURBATION > 0:
            neg_data.append(sub_data[ninst:, :])
    orig_data = torch.cat(orig_data, dim=0)
    if cfg is not None and cfg.AVA.PERTURBATION > 0:
        neg_data = torch.cat(neg_data, dim=0)
        return orig_data, neg_data, ninst
    return orig_data, None, ninst