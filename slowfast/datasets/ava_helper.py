...
def load_clip_lists(cfg, is_train):
    """
    ...
    logger.info(
        "Finished loading video paths from: %s" % ", ".join(list_filenames)
    )
...
def load_boxes_and_labels(cfg, mode):
    ...
    if cfg.MODEL.IS_FULL or mode != 'train':
        ann_filenames = [
        ...
        d_ann_filenames = [
            os.path.join(cfg.AVA.ANNOTATION_DIR, filename)
            for filename in pred_lists
        ]
        d_ann_is_gt_box = [False] * len(pred_lists)
    ...
    if (not cfg.MODEL.IS_FULL) and mode == 'train':
        all_boxes, count, unique_box_count = parse_bboxes_file(
            ann_filenames=d_ann_filenames,
            ann_is_gt_box=d_ann_is_gt_box,
            detect_thresh=detect_thresh,
            boxes_sample_rate=boxes_sample_rate,
            gt_all_boxes=all_boxes,
            clip_duration=int(cfg.AVA.CLIP_DURATION),
        )
...
def get_keyvideo_data(boxes_and_labels):
    """
...
def parse_bboxes_file(
    ann_filenames, ann_is_gt_box, detect_thresh, boxes_sample_rate=1, gt_all_boxes=None, clip_duration=1,
):
    """
        ...
        for filename, is_gt_box in zip(ann_filenames, ann_is_gt_box):
            with g_pathmgr.open(filename, "r") as f:
                for line in f:
                    ...
                    label = -1 if (row[6] == "" or not is_gt_box ) else int(row[6])
                    ...
                    if box_key not in all_boxes[video_name][frame_sec]:
                        all_boxes[video_name][frame_sec][box_key] = [box, [], score]
                        unique_box_count += 1
        ...
        for filename, is_gt_box in zip(ann_filenames, ann_is_gt_box):
            with g_pathmgr.open(filename, "r") as f:
                for line in f:
                    ...
                    gtlabel = []
                    clip_loc_start = (frame_sec - 902) // clip_duration * clip_duration + 902
                    for fr_sec in range(clip_loc_start, clip_loc_start + clip_duration, 1):
                        if fr_sec not in gt_all_boxes[video_name].keys():
                            continue
                        for gt_boxes in gt_all_boxes[video_name][fr_sec].keys():
                            gtlabel += gt_all_boxes[video_name][fr_sec][gt_boxes][1]
                    if len(gtlabel) == 0:
                        gtlabel = [-1]
                    gtlabel = list(set(gtlabel))
                    all_boxes[video_name][frame_sec][box_key][1] = gtlabel
                    count += 1

    return all_boxes, count, unique_box_count

def load_scene( cfg, video_to_idx_name, mode='train'):
    if mode != "train" or cfg.AVA.TRAIN_SCENE == "":
        return None
    scene_cut = {}
    for id, v in enumerate(video_to_idx_name):
        scene_dd = np.genfromtxt(cfg.AVA.TRAIN_SCENE+v+'-Scenes.csv',dtype=np.str,delimiter=",")
        scene_dd = scene_dd[:, 3].astype(np.float)
        scene_cut[id] = [np.argmax(scene_dd >= i) for i in range(902,1799)]
    return scene_cut