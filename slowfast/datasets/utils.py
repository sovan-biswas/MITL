...
def retry_load_video(image_paths, retry=10, backend="pytorch"):
    """
    ...
    for i in range(retry):
        v = cv2.VideoCapture(image_paths)

        if v.isOpened():
            imgs = [v.read()[1] for i in range(int(v.get(cv2.CAP_PROP_FRAME_COUNT)))]
            ...
            time.sleep(1.0)
        if i == retry - 1:
            raise Exception("Failed to load video {}".format(image_paths))
...
def loader_worker_init_fn(dataset):
    ...
    Args:
        dataset (torch.utils.data.Dataset): the given dataset.
    """
    return np.random.seed(int(time.time()))
