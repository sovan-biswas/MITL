...
class ResNetRoIHead(nn.Module):
    ...
    def __init__(
        ...
        # self.uncertainity = nn.Linear(sum(dim_in), num_classes, bias=True)
        self.contrastive = contrastive
        if self.contrastive:
            if hasattr(self, "dropout"):
                self.contrastive_projection = nn.Sequential(
                    # nn.Linear(sum(dim_in), 512),
                    nn.Linear(sum(dim_in), 512),
    ...
    def forward(self, inputs, bboxes):
        ...
        if self.contrastive:
            if hasattr(self, "dropout"):
                y = self.contrastive_projection(self.dropout(x))
            else:
                y = self.contrastive_projection(x)
